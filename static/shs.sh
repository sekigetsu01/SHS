#!/bin/sh
# SHS - Sekigetsu's Host Setup
# Run as root on a fresh Arch/Artix installation.


# CONFIGURATION

DOTFILES="https://github.com/sekigetsu01/hostrice.git"
PROGSFILE="https://raw.githubusercontent.com/sekigetsu01/SHS/main/static/progs.csv"
AURHELPER="yay"
BRANCH="main"
export TERM=ansi
...
info() { whiptail --title "SHS" --infobox "$1" 8 70; }


# DIALOG PROMPTS

welcome() {
	whiptail --title "Welcome to SHS!" \
		--msgbox "Welcome to Sekigetsu's Host Setup.\n\nThis script will automatically set up a full desktop environment for virtualization.\n\n-Sekigetsu" 10 60
...
check_existing_user() {
	id -u "$name" >/dev/null 2>&1 && \
		whiptail --title "User already exists" \
			...
			--yesno "The user '$name' already exists.\n\nSHS will overwrite conflicting dotfiles and settings but will NOT touch personal files (Documents, Videos, etc).\n\nThe password will also be updated." 12 65
}
...
create_directories() {
	...
	for dir in Downloads Documents Passwords ISOs VMs github; do
...
aur_install() {
	...
	whiptail --title "SHS – Installing ($n/$total)" --infobox "AUR: $1\n$2" 8 70
...
git_make_install() {
	...
	whiptail --title "SHS – Installing ($n/$total)" --infobox "git+make: $progname\n$2" 8 70
...
pipx_install() {
	...
	whiptail --title "SHS – Installing ($n/$total)" --infobox "pipx: $1\n$2" 8 70
...
flatpak_install() {
	whiptail --title "SHS – Installing ($n/$total)" --infobox "flatpak: $1\n$2" 8 70
	pkg flatpak
...
pacman_install() {
	whiptail --title "SHS – Installing ($n/$total)" --infobox "pacman: $1\n$2" 8 70
	pkg "$1"
}
...
configure_system() {
	...
	# Allow tearing for gaming optimization
...
trap 'rm -f /etc/sudoers.d/shs-temp' HUP INT QUIT TERM PWR EXIT
echo "%wheel ALL=(ALL) NOPASSWD: ALL" > /etc/sudoers.d/shs-temp
